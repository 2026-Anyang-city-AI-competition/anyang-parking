...
OUT_CSV = (
    ROOT
    / "reports/tables/a14_2_weather_model.csv"
)

FREQ = "5min"
HORIZONS = [15, 30, 60, 120]

# A12.1과 동일: 전체 시간축 마지막 20%를 test
TEST_RATIO = 0.20
...
# Weather merge
# =========================================================
def merge_weather(base, weather):
...
def main():
    ...
    print("=" * 78)
    print("A14-2 Weather Forecast Ablation")
    print("=" * 78)
    ...
    for horizon in HORIZONS:
        ...
        # 동일 rows 비교
        #
        # weather 포함해서 필요한 값이 있는 행만 사용.
        # 현재 weather coverage 100%라 사실상 영향 없음.
        # -------------------------------------------------
        required = list(
            dict.fromkeys(
                weather_features
        ...
        print(
            "A14 Time+Weather  "
            f"MAE={m_weather['MAE']:.3f} "
            f"Median={m_weather['MedianAE']:.3f} "
            f"P90={m_weather['P90AE']:.3f}"
        )
        ...
        importance = pd.Series(
            model_weather.feature_importances_,
        ...
        print()
        print("[Weather importance]")

        for feature in weather_cols:

            print(
                f"{feature:15s} : "
                f"{importance.get(feature, 0)}"
            )
