#!/usr/bin/env python3
"""
Rolling origin evaluation for U9-2.
"""
import pandas as pd
import numpy as np
import json
...
def _add_opr(df, h, meta):
    ...
    # This is a simplified version; we assume the function exists in temporal.py
...
def rolling_origin_evaluation():
    ...
    # We'll use the first 4 days for initial train, then test on day 4,5,6,7 (0-indexed)
    # Actually, the example: fold1: train 0-2 (inclusive) -> test 3
    # We'll adjust to have at least 3 days train and 1 day test, expanding.
    folds = []
    ...
    for fold_info in folds:
        ...
        for h in HORIZ_GRID:
            ...
            # We'll use the train and test masks as is, but we need to set nx to NaN for rows where the shifted time is in the test period or later.
            # We'll do a simple approximation: if the original ts_kst is in test, then nx is definitely in test or later (since h>0).
            # We'll set nx to NaN for rows where ts_kst is in test.
            # Also, we need to consider rows where ts_kst is in train but ts_kst + h is in test.
            # We'll do a more accurate check: compute the shifted timestamp and see if its date is in test.
            # However, for simplicity and given the small h (max 2 hours), we'll assume that if the date is the same, it's okay.
            # We'll just mask out rows where the original ts_kst is in test.
            # This is not perfect but will be better than nothing.

            # Set nx to NaN for rows where ts_kst is in test (because then ts_kst + h is in test or later)
            d_copy.loc[test_mask, "nx"] = np.nan
            d_copy.loc[test_mask, "y"] = np.nan

            train_df = d_copy[train_mask].copy()
            test_df = d_copy[test_mask].copy()

            # Drop rows where nx is NaN (due to shift or our masking)
            train_df = train_df.dropna(subset=["nx"])
            test_df = test_df.dropna(subset=["nx"])

            if len(train_df) == 0 or len(test_df) == 0:
                print(f"  Warning: No data for horizon {h} in fold {fold_info['fold']}")
                continue
            ...
            # Router selection: we'll split the train data into base and val (last 20% of train dates for validation).
            train_dates_sorted = sorted(train_df.ts_kst.dt.date.unique())
            if len(train_dates_sorted) >= 2:
                split_idx = max(1, int(len(train_dates_sorted) * 0.8))
                base_dates = train_dates_sorted[:split_idx]
                val_dates = train_dates_sorted[split_idx:]
...
if __name__ == "__main__":
    ...
    df.to_csv("/Users/chabee/new/New_Source/2026안양시공모전/anyang-parking/reports/tables/u9_rolling_origin.csv", index=False)
    # Also create a markdown table
    with open("/Users/chabee/new/New_Source/2026안양시공모전/anyang-parking/reports/tables/u9_rolling_origin.md", "w") as f:
