#!/usr/bin/env python3
"""
Rolling origin evaluation for U9-2, version 2 with proper purging.
"""
import pandas as pd
import numpy as np
import json
...
def rolling_origin_evaluation():
    ...
    for fold_info in folds:
        ...
        for h in HORIZ_GRID:
            # We'll create features and target separately for train and test to avoid leakage.
            # First, create a copy of d for feature engineering (without the target shift yet).
            d_feat = d.copy()
            # Add operating features for target time (ts + h) - we need the meta for this.
            meta_dict = {r["parking_id"]: r for r in L.to_dict("records")}
            d_feat = _add_opr(d_feat, h, meta_dict)

            # Now, split into train and test by date.
            train_feat = d_feat[train_mask].copy()
            test_feat = d_feat[test_mask].copy()

            # For each set, we create the target nx by shifting the occurrence.
            # But we must ensure that the shifted time is within the same set.
            # We'll do this by, for each set, shifting and then dropping rows where the shifted time is not in the set.
            # However, note that the shift is within each parking_id group.

            # We'll create the target in a temporary dataframe for each set.
            def create_target(df_set, set_name):
                # Make a copy to avoid SettingWithCopyWarning
                df_set = df_set.copy()
                # Shift the occurrence to get nx (occupancy at ts_kst + h)
                df_set["nx"] = df_set.groupby("parking_id")["occ"].shift(-(h//5))
                # Now, we need to drop rows where the shifted time is not in the same set.
                # We can do this by checking that the ts_kst of the shifted row is in the set.
                # But note: after shifting, the index is messed up. We'll instead create a column for the shifted timestamp.
                # We'll compute the shifted timestamp as ts_kst + h minutes.
                df_set["ts_kst_plus_h"] = df_set["ts_kst"] + pd.Timedelta(minutes=h)
                # Now, we keep only rows where the shifted timestamp's date is in the set's dates.
                if set_name == "train":
                    valid_dates = fold_info['train_dates']
                else:
                    valid_dates = [fold_info['test_date']]
                # Filter
                df_set = df_set[df_set["ts_kst_plus_h"].dt.date.isin(valid_dates)].copy()
                # Now compute y = nx - occ
                df_set["y"] = df_set["nx"] - df_set["occ"]
                # Drop the helper columns
                df_set = df_set.drop(columns=["ts_kst_plus_h"])
                return df_set

            train_df = create_target(train_feat, "train")
            test_df = create_target(test_feat, "test")

            if len(train_df) == 0 or len(test_df) == 0:
                print(f"  Warning: No data for horizon {h} in fold {fold_info['fold']} (train:{len(train_df)}, test:{len(test_df)})")
                continue

            # Feature columns (exclude the target and helper columns)
            F = [c for c in train_df.columns if c in INTX or c.startswith("opr_")]
            # Note: we must ensure that the feature columns are present in both train and test.
            # They should be because we used the same d_feat.
