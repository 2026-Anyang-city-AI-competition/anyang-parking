...
빈 파일·누락 규칙·불량 파일은 예측을 허용하지 않는다. 현재 DB 진단으로
89곳 × 3개 요일군 = 267행을 적재했고, 68곳의 검증된 시간대에서만 예측을
연다. 고정 피드 21곳과 경직·이상 시간대는 예측값을 null로 반환하고
위치·요금 정보만 제공한다.
...
현재 시작 주소: `http://35.229.35.13`

- GCE `anyang-poller` 한 대에 5분 폴러, FastAPI 1 worker, Nginx를 함께 실행한다.
- Nginx가 `/`에서 React 빌드를 제공하고 `/api/`를 `127.0.0.1:8000`으로 프록시한다.
- `parking.db`는 VM 영구 디스크에 두고 WAL로 읽기·쓰기 충돌을 막는다.
- `anyang-backup.timer`는 매일 03:20 KST에 SQLite 일관 스냅샷을 만들고 7개를 보관한다.
- `anyang-health.timer`는 5분마다 모델·관측 신선도·조사 규칙·예측 게이트를 확인한다.
- VM 실행 패키지는 `requirements-api.txt`의 모델 생성 버전으로 고정한다.
- systemd·Nginx 정본은 `scripts/deploy/`에 있다. 키는 코드·유닛 파일에 넣지 않고
  서버의 `/etc/anyang-parking/api.env`(0600)에서만 읽는다.

현재 주소는 HTTP IP 주소다. 운영 도메인을 결정하면 DNS를 정적 IP에 연결하고
HTTPS 인증서와 카카오 OAuth 콜백 주소를 동시에 설정한다.
