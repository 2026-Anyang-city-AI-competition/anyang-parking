...
- [ ] 프런트 출발시각 바텀시트: 지금~120분을 5분 단위로 선택
- [ ] 프런트 주차시간 바텀시트: 빠른 선택 + 5분 단위 직접 설정
- [ ] 현재 위치 또는 직접 입력한 출발지 좌표를 추천 API에 전달
- [ ] 미래 출발도 공통 ETA 하나가 아니라 후보별 ETA를 사용
- [ ] 후보별 도착·출차·예측시각과 요금·출입 판정의 엔드투엔드 테스트
...
- [ ] 목적지 입력창 오른쪽 지도 아이콘과 접근성 라벨 추가
- [ ] 목적지 선택 지도·중앙 핀·현재 위치 이동 버튼 구현
- [ ] 지도 탭/핀 이동 좌표를 주소로 바꾸는 역지오코딩 API 또는 서버 프록시 추가
- [ ] 검색으로 고른 목적지와 지도에서 고른 목적지가 같은 상태를 공유하도록 연결
- [ ] 위치 권한 거부·역지오코딩 실패·지도 SDK 실패 상태 처리
