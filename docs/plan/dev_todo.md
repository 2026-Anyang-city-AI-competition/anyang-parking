...
- [x] 요청 시 모델 재구성을 제거하고 백그라운드 갱신·경로 캐시로 추천 응답시간 단축
  — 15초 주기 스냅샷 갱신, 모델 파일 무중단 교체, 차량 경로 5분 캐시. 동일 조건 실측
  3.50초 → 0.51초(약 85% 단축). 실제 새 폴링이 발생한 요청은 갱신 완료 후 예측한다.
...
- [x] 지도 `확대`·`축소`·`내 위치` 버튼 — 추천 결과 실제 지도에 Leaflet 확대·축소와 명시적 위치 권한 버튼, 지도 높이 전환을 구현
...
- [x] 현재 좌표 투영식으로 그린 모형 지도를 실제 지도 SDK로 교체 — Leaflet + OpenStreetMap
- [x] 목적지 핀과 순위 번호 주차장 핀 구현
- [x] 핀↔카드 양방향 선택·스크롤·강조 동기화 — 카드의 상세 화살표는 상세 화면 진입
- [x] 정렬 탭 변경 시 핀 순위와 카드 순위를 함께 갱신
- [x] 지도 로딩 실패 시 목록은 계속 사용할 수 있는 폴백 제공
- [x] 390×844 모바일 브라우저에서 검색→시간설정→추천→지도 핀→상세 화면 E2E 및 가로 넘침 없음 확인
